                ...
                <div class="col-lg-8">
                    <div class="tp-postbox-wrapper">
                        ...
                        <template v-for="blog in currentItems" :key="blog.id">
                            ...
                            <blog-item-list-view v-else :blog="blog" />
                        </template>
                    ...
                    <div class="tp-event-inner-pagination tp-postbox-item-pagination">
                        <div class="tp-dashboard-pagination">
                            <div class="tp-pagination">
                                <ui-pagination :total-items="blog_lists_data.length" :items-per-page="5"
                                    :max-pages-shown="5" v-model="currentPage" @pageChange="updatePage" :is-center="false" />
                            </div>
                        </div>
                    </div>
                </div>
...
const length = computed(() => blog_lists_data.length);
const { currentPage, endItem, startItem, updatePage } = usePagination(length, 5);

const currentItems = computed(() =>
    blog_lists_data.slice(startItem.value, endItem.value)
);
