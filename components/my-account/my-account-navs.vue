<template>
    <nav>
        <div class="nav nav-tabs tp-tab-menu flex-column" id="profile-tab" role="tablist">
            <button v-for="item in navItems" :key="item.id" :class="['nav-link', { active: item.active }]" :id="item.id"
                data-bs-toggle="tab" :data-bs-target="item.target" type="button" role="tab"
                :aria-controls="item.target.slice(1)" :aria-selected="item.active ? 'true' : 'false'"
